...
class EnvironmentWorkerProcess(ProcessBase):
    ...
    def set_transfer_buffers(self, torch_transfer_buffers: TensorDict):
        ...
        self.actions = self._torch_transfer_buffers["actions"].numpy()
...
def _create_worker_configs(config: Config):
    ...
    # If we have a lot of scenes compare to envs,
    # divide things up as it won't bias sampling much
    # but will decrease load time and memory usage.
    if num_environments < len(scenes) / 4:
        ...
        for idx in range(num_environments):
            scene_splits[idx] = scene_splits[idx][0:1]
    else:
        scenes = None
        scene_splits = None
