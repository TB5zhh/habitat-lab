...
def non_ragged_copy_fn(new_reqs, dst_max_step, tgt_inds, src, dst):
    ##
    # NB: This loop is faster than something fancy with index_copy_
    # or gather_!  I think that's because copy_ is much faster and
    # async
    for i, env_idx in enumerate(new_reqs):
        step_idx = tgt_inds[env_idx].item()
        if 0 <= step_idx <= dst_max_step:
            dst[step_idx, env_idx].copy_(src[i])


def ragged_copy_fn(new_reqs, tgt_inds, src, dst):
    ##
    # NB: This loop is faster than something fancy with index_copy_
    # or gather_!  I think that's because copy_ is much faster and
    # async
    for i, env_idx in enumerate(new_reqs):
        dst_idx = tgt_inds[env_idx].item()
        if dst_idx >= 0:
...
class InferenceWorkerProcess(ProcessBase):
    ...
    policy_name: str
    policy_args: Tuple
    actor_critic_tensors: List[torch.Tensor]
    device: torch.device
    ...
    def _update_storage_non_ragged(
    ...
    def step(self) -> Tuple[bool, List[int]]:
        ...
        with self.timer.avg_time("update storage"):
            ...
            if self._variable_experience:
                self._update_storage_ragged(prev_step, current_step, my_slice)
            else:
                self._update_storage_non_ragged(
...
class InferenceWorker(WorkerBase):
    def __init__(self, mp_ctx: BaseContext, *args, **kwargs):
        super().__init__(mp_ctx, InferenceWorkerProcess, *args, **kwargs)
