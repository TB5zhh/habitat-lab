...
class VERTrainer(PPOTrainer):
    ...
    def _init_train(self, resume_state):
        ...
        config = self.config
        if (
            hasattr(config, "PREFIX")
            and config.PREFIX not in config.TENSORBOARD_DIR
        ):
            real_tb_dir = os.path.join(config.TENSORBOARD_DIR, config.PREFIX)
            config.defrost()
            # Inject the prefix into all of the filepaths
            config.VIDEO_DIR = os.path.join(config.VIDEO_DIR, config.PREFIX)
            config.CHECKPOINT_FOLDER = os.path.join(
                config.CHECKPOINT_FOLDER, config.PREFIX
            )
            if not os.path.exists(config.VIDEO_DIR) and rank0_only():
                os.makedirs(config.VIDEO_DIR)
            if not os.path.exists(config.CHECKPOINT_FOLDER) and rank0_only():
                os.makedirs(config.CHECKPOINT_FOLDER)

            config.TENSORBOARD_DIR = real_tb_dir
            if not os.path.exists(config.TENSORBOARD_DIR) and rank0_only():
        ...
        self.preemption_decider = PreemptionDeciderWorker(
            ...
            get_main_addr(),
            get_free_port_distributed(
                "preemption", tcp_store if self._is_distributed else None
            ),
            world_rank,
            world_size,
            self.config,
        ...
        self.report_worker = ReportWorker(
            self.mp_ctx,
            get_free_port_distributed(
                "report", tcp_store if self._is_distributed else None
            ),
            self.config,
            self.queues.report,
            self._my_t_zero,
        ...
        inference_worker_args = (
            ...
            list(self.actor_critic.all_policy_tensors()),
            self.device,
        ...
        self._all_workers.extend(
            self.environment_workers
            + self.inference_workers
            + [self.report_worker, self.preemption_decider]
        )
